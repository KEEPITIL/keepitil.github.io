...
(function () {
            ...
            return rpc('vs_add_entry_media', { p_entry: entryId, p_url: d.media_url,
              ...
              .then(function (sub) {
                ...
                return rpc('cho_log_action', { p_action: 'submit_create_entry', p_object_type: 'vs_entry',
                  ...
                  .then(function () {
                    /* ⚠ VERIFY BEFORE CLAIMING (Founder §2: requested ≠ attempted ≠ completed ≠
                       verified). The RPC returning 200 means the call was accepted, not that the
                       entry is in the state the user thinks. Read the row back and report ITS
                       status — if submission did not stick, say so instead of congratulating
                       someone on an entry that is still a draft. */
                    return fetch(KIL_SUPA_URL + '/rest/v1/vs_entries?select=id,status,payment_status'
                                 + '&id=eq.' + encodeURIComponent(entryId), { headers: H })
                      .then(function (rr) { return rr.ok ? rr.json() : null; })
                      .then(function (rows) {
                        var row = (rows && rows[0]) || null;
                        var st = row ? String(row.status || '') : '';
                        choClearFlow();
                        if (!row) {
                          return { title: 'Submitted — could not verify',
                                   text: 'The submission was accepted but I could not read the entry back, '
                                       + 'so I will not claim it is done. Check CREATE › My entries.' };
                        }
                        var landed = /submit|review|pending/i.test(st);
                        if (landed) {
                          setTimeout(function () { location.href = '/create/?view=entry&e=' + entryId; }, 1400);
                        }
                        return { title: landed ? 'Submitted ✓' : 'Saved, not submitted',
                                 text: landed
                                   ? 'Verified: your entry is recorded as "' + st + '". It now goes through '
                                     + 'moderation before it appears publicly — nothing is public until it passes. '
                                     + 'Opening your entry…'
                                   : 'The entry exists but its status is "' + st + '", not a submitted state. '
                                     + 'I am not going to tell you it is in when it is not — open CREATE › My '
                                     + 'entries to finish it.' };
                      });
                  });
              });
          })
  ...
  /* ── CHO DRAFTING (Founder §4) ────────────────────────────────────────────────────────────
     Helps a user write copy for an event description, a CREATE entry or an article.

     ⚠ DRAFTING CANNOT PUBLISH, AND CANNOT BYPASS MODERATION. It produces TEXT in the
     conversation and nothing else — it writes no row, sets no status and touches no permission.
     Whatever the user does with the text afterwards goes through the same submission,
     moderation and approval it always did. Article publication in particular stays
     Founder-approval-only; a well-written draft is still a draft.

     The wording is generated by the existing agent/brain chain, not invented here — this
     function only recognises the intent and frames the request so the answer comes back as
     copy rather than as a conversation about copy. */
  function choDraftIntent(t) {
    if (!/\b(draft|write|help me write|copy|description|blurb|bio|caption)\b/i.test(t)) return null;
    var kind = /\barticle\b/i.test(t) ? 'article'
             : /\bevent\b/i.test(t) ? 'event description'
             : /\b(entry|create|submission)\b/i.test(t) ? 'CREATE entry description'
             : null;
    if (!kind) return null;
    return {
      title: 'Drafting a ' + kind,
      text: 'Tell me the essentials and I will write it: '
          + (kind === 'article'
              ? 'who or what it is about, and anything you want mentioned. I will produce a draft — '
                + 'publishing it still needs Founder approval, which I cannot give.'
              : kind === 'event description'
              ? 'the artist or theme, the venue and the vibe. The draft is yours to paste into the '
                + 'event form; it does not create or publish anything by itself.'
              : 'what you made, how you made it, and what it is for. The draft goes in your entry — '
                + 'it does not submit it, and it does not skip moderation.')
    };
  }
